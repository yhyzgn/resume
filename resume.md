...
​	【`2019.08`-`2023.09`】【`Java`开发工程师 / `Golang`开发工程师 / 架构师 / 技术经理】
...
​	带领技术团队从`0`-`1`完成 “一部手机管旅游” 平台的建设，包括平台架构、技术选型、中间件开发集成，业务功能开发、项目部署等工作。平台采用微服务模式架构，各模块之间开合有度，满足 “高内聚低耦合” 的软件设计原则；并以分布式模式部署运行，保障平台的高扩展、高可用、高性能特性。
...
​	【`2023.09`- `2023.09`】【`Java`开发工程师 / 架构师】
...
​	已持续稳定运行`3`年且无需技术调整，可便捷动态地插件化部署（新增/升级/停止）相关业务系统。
...
​	工作内容较繁杂，包括项目管理、产品需求管理、技术选型、`Java`后端开发和`Android`客户端开发等。
